...
- **Map tiles**: OpenStreetMap's own servers refuse requests that don't come from a proper website — opening `index.html` off your disk is one of those, and you get grey "Access blocked / 403" squares. CARTO allows it but now stamps **API KEY REQUIRED** across every tile unless you have a key. So the default is **Esri**, which needs no key and no account: *Streets*, *Topographic* and *Satellite* in the selector in the *Where they are* header. Your pick is remembered per device, and if a provider stops answering the map moves itself down the list and says so.
- **CARTO key (optional)**: the two CARTO styles are kept in the list for anyone who wants them. A free key from carto.com → Developers → API Keys, pasted into **⚙︎ Sync → CARTO basemap key**, removes the watermark; the page appends it as `?api_key=` on the tile requests. Without a key those two styles work but stay stamped. Nothing else in the page needs it.
